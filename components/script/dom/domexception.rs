...
impl DOMException {
    fn get_error_data_by_code(code: DOMErrorName) -> (DOMString, DOMString) {
        let message = match &code {
            ...
            DOMErrorName::TransactionInactiveError => "A request was placed against a transaction which is currently not active, or which is finished.",
            DOMErrorName::ReadOnlyError => "The mutating operation was attempted in a \"readonly\" transaction.",
            DOMErrorName::VersionError => "An attempt was made to open a database using a lower version than the existing version.",
